...
func TestStreamProgress_VisibleAfter2s(t *testing.T) {
	...
	(&m).updateStreamProgress(512)
	if !m.showStreamProgress() || m.renderStreamProgress(m.width) == "" {
		t.Fatal("visible")
	}
}
