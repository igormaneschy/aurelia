...
Aurelia is the **product layer** on top of the PI SDK engine. When the PI SDK or a PI-loaded MCP already owns a capability, Aurelia adapts or orchestrates it rather than reimplementing it. When the capability is product-specific — continuity, operational memory, policy, UX, or workflow state — Aurelia remains the source of truth.
...
| Wiki Memory | Transversal memory through `ai-memory` MCP | PI MCP configuration, not Aurelia internals |
...
| Operational Memory | User, topic, cwd overlay and team context used by Telegram UX/prompt/continuity | `internal/memoryux/`, `internal/dream/` |
| Memory Extraction | Nudge/Dream for operational context and suggestions; Wiki writes only via verified PI/ai-memory path | `internal/dream/` — Dreamer, safeMemoryWriter |
