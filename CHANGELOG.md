...
### Fixed
- Bridge `list-models` agora força refresh com `force:true` (antes `allowNetwork` sem `force` deixava `opencode-go` em 25 modelos stale vs 33 no remoto). `atualiza modelos` volta a trazer `hy4-preview`, `mimo-v2-omni`, `grok-4.5`, `qwen3.5-plus` etc (239→290 total).

## [0.43.1] - 2026-09-02

### Changed
- Toolchain Go 1.26.6 → 1.27 (`go.mod` + CI `setup-go` 1.27, docs).
...
### Added
- Bridge priority queue: gate stdin writes so interactive Telegram/TUI queries run
  ahead of cron jobs and dream/nudge background work; cancel/ping bypass the queue.

### Fixed
- Dream/nudge circuit breaker: pause background API calls for 30 minutes after
  401/402/429 (auth, billing, rate-limit) to avoid burning tokens when the
...
### Fixed
- TUI now shares dreamer, nudgeBuffer and memoryCache with Telegram.
  Conversations in TUI generate automatic memory via nudge/review and
  reuse the mtime cache between sends (#43)
...
### Changed
- **Docs: README atualizado** — reflete o estado atual do projeto, incluindo TUI local, anexos, imagens, comandos, build/deploy e versão corrente.
